...
const AdminSystemMonitor: React.FC = () => {
  ...
  const fetchSystemMetrics = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/system/metrics`);
  ...
  const fetchSystemLogs = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/system/logs`);
      const result = await response.json();
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      case 'stable': return <div className="w-4 h-4 border-2 border-gray-400 border-t-transparent border-r-transparent border-l-transparent"></div>;
      default: return null;
    }
  };
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
                  {React.createElement(getIconComponent(metric.icon), { className: "w-5 h-5 text-gray-600" })}
                </div>
                <div>
              ...
              <div key={log.id} className="p-4 hover:bg-gray-50 transition-colors">
                <div className="flex items-start space-x-3">
                  <div className={`w-2 h-2 rounded-full mt-2 ${
                    ...
                    log.level === 'error' ? 'bg-red-500' : 'bg-red-700'
                  }`}></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button className="p-4 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors">
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              <p className="text-sm font-medium text-blue-900">Restart Services</p>
              <p className="text-xs text-blue-600">Restart all system services</p>
            </div>
          </div>
        </button>
        ...
        <button className="p-4 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors">
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              <p className="text-sm font-medium text-green-900">Backup Database</p>
        ...
        <button className="p-4 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors">
          <div className="flex items-center space-x-3">
            ...
            <div className="text-left">
              <p className="text-sm font-medium text-purple-900">Performance Test</p>
